...
function RecipeView(props) {
    ...
    return(<div style={{'padding-bottom': '1em'}} key={ingredient.name}>{ingredient.quantity} {ingredient.measurement} {ingredient.name}</div>)
    })
    const directions = props.data.instructions.map((instruction, index) => {
    return(<div style={{'padding-bottom': '1em'}} key={index}>Step {index+1}: {instruction}</div>)
    })
return(<div>
    ...
    <div className="recipe-page-description" style={{'padding-bottom': '1em'}}>{props.data.description}</div>
    <div className="ingredients-container">
        <div style={{'display': 'flex', flexDirection: 'row', justifyContent: 'space-evenly'}}>
        <div style={{'padding-bottom': '1em'}}>Time required: {props.data.time} mins</div>
        <div style={{'padding-bottom': '1em'}}>Serves: {props.data.servings}</div>
        </div>
        <div style={{'padding-bottom': '1em'}}>Ingredients: </div>
        <div>{ingredients}</div>
        <div style={{'padding-bottom': '1em'}}>Instructions: </div>
        <div>{directions}</div>
    </div>
